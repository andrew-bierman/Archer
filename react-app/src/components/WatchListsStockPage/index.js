  ...
  const handleAddStockToWatchlistSubmit = async (e) => {
    ...
    Object.keys(selectedWatchlists).forEach((watchlistId) => {
      if (selectedWatchlists[watchlistId]) {
        ...
        !selectedWatchlists[watchlistId] &&
        preCheckedWatchlists[watchlistId]
      ) {
        dispatch(removeStockFromWatchlistThunk(watchlistId, stockId));
      } else {
        return;
    ...
    <div className="watchlist-modal-container">
      ...
      <div>
        <form onSubmit={(e) => handleAddStockToWatchlistSubmit(e)}>
              ...
              watchlists?.map((watchlist) => (
                <>
                  ...
                  watchlist?.id !== 0 ? (
                    <div
                      key={watchlist.id}
                      className="stock-page-watchlist-modal-watchlist-individual"
                    >
                      <label className="checkbox mr-2">
                        <input
                          ...
                          id={watchlist.id}
                          value={watchlist.id}
                          className="watchlist-modal-individual-checkbox"
          ...
          <button className="button">Update Watchlists</button>
        </form>
      </div>
    </div>
  );
